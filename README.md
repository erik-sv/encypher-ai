...
Try EncypherAI directly in your browser with our interactive Google Colab notebook. The notebook demonstrates all the core features including metadata embedding, extraction, HMAC verification, and tampering detection.
...
# Note: Use the same secret key provided during embedding
# Returns tuple: (metadata_dict, is_verified)
metadata_dict, is_verified = UnicodeMetadata.verify_metadata(
    text=encoded_text,
...
if is_valid:
    print(f"Model: {verified_metadata.get('model_id')}")
    print(f"Timestamp: {verified_metadata.get('timestamp')}")
    print(f"Custom field: {verified_metadata.get('custom_field')}")
    print(f"Version: {verified_metadata.get('version')}")
```
...
# After processing all chunks, you can verify the full response
from encypher.core.unicode_metadata import UnicodeMetadata
is_valid, verified_metadata = UnicodeMetadata.verify_metadata(
    text=full_response,
    hmac_secret_key="your-secret-key"
...
if is_valid:
    print(f"Streaming content verified: {verified_metadata}")
```
...
if "custom" in metadata:
    ...
    version = metadata["custom"]["version"]  # "1.1.0"
...
# Assuming hmac_secret_key was set during embedding (e.g., via ENV or passed directly)
hmac_secret_key = "your-secret-key" # Or fetch from config/env

encoded_text = "AI-generated text with embedded metadata..."

# Returns tuple: (metadata_dict, is_verified)
metadata, is_valid = UnicodeMetadata.verify_metadata(encoded_text, hmac_key)
